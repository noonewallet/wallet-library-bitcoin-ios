...
public class BitcoinTransactionSerializer {
    ...
    public func transaction<Transaction: BitcoinTransactionProtocol>(from data: Data) throws -> Transaction {
        ...
        let (txinputs, inputsCountLengthBytes) = (try? inputs(from: data.subdata(in: length..<data.count))) ?? ([], 0)
        
        length += txinputs.map({ $0.payload.count }).reduce(0, +) + inputsCountLengthBytes
        
        let (txoutputs, outputsCountLengthBytes) = (try? outputs(from: data.subdata(in: length..<data.count))) ?? ([], 0)
        
        length += txoutputs.map({ $0.payload.count }).reduce(0, +) + outputsCountLengthBytes
    ...
    public func inputs(from data: Data) throws -> ([BitcoinTransactionInput], Int) {
        ...
        _data = _data.subdata(in: inputsCount.length..<_data.count)
        ...
        return (_inputs, inputsCount.length)
    }
    
    
    public func outputs(from data: Data) throws -> ([BitcoinTransactionOutput], Int) {
        ...
        _data = _data.subdata(in: outputsCount.length..<_data.count)
        ...
        return (_outputs, outputsCount.length)
    }
