...
public class BitcoinTransaction: BitcoinTemplateTransaction, BitcoinTransactionProtocol {
    ...
    public func sign(keys: [KeySecp256k1], signatureHashType: SignatureHashType = SignatureHashType(btc: .sighashAll)) throws -> BitcoinTransaction {
        ...
        for (index, input) in inputs.enumerated() {
            ...
            let signature = SignatureECDSA.sign(nonce: .RFC6979, output: .DER, data: hash, key: key.data, recid: &recid)
...
extension BitcoinTransaction: CustomStringConvertible {
    ...
    public var description: String {
        ...
        VERSION: \(version)
        MARKER: \(String(describing: settings.witness?.marker))
        FLAG: \(String(describing: settings.witness?.flag))
        INPUTS: \(inputs)
        OUTPUTS: \(outputs)
        WITNESS: \(String(describing: witness?.hex))
        LOCKTIME: \(lockTime)
        PAYLOAD: \(payload.hex)
        """
    }
